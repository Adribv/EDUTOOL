...
const DisciplinaryFormAcknowledge = () => {
  ...
  const acknowledgeMutation = useMutation({
    mutationFn: (data) => disciplinaryAPI.parentAcknowledge(formId, data),
    onSuccess: () => {
      setSnackbar({ open: true, message: 'Form acknowledged successfully', severity: 'success' });
      setTimeout(() => navigate('/parent/disciplinary-forms'), 2000);
    },
    onError: (error) => {
      setSnackbar({ 
        open: true, 
  ...
  if (form.parentAcknowledgment?.acknowledged) {
    return (
      <Box p={3}>
        <Alert severity="info" icon={<CheckCircle />}>
          You have already acknowledged this disciplinary form on {new Date(form.parentAcknowledgment.date).toLocaleDateString()}.
        </Alert>
        <Button
          startIcon={<ArrowBack />}
          onClick={() => navigate('/parent/disciplinary-forms')}
          sx={{ mt: 2 }}
        >
          Back to Forms
        </Button>
    ...
    <Box p={3}>
      ...
      <Box display="flex" alignItems="center" mb={3}>
        ...
        <Typography variant="h4" display="flex" alignItems="center">
          <FamilyRestroom sx={{ mr: 2, color: 'primary.main' }} />
          Parent Acknowledgment - Disciplinary Form
        </Typography>
      </Box>
          ...
          <Paper sx={{ p: 3 }}>
            ...
            <Card sx={{ mb: 2 }}>
              <CardContent>
                ...
                {form.studentAcknowledgment?.acknowledged ? (
                  <Alert severity="success" icon={<CheckCircle />}>
                    Student acknowledged this form on {new Date(form.studentAcknowledgment.date).toLocaleDateString()}
                    {form.studentAcknowledgment.comments && (
                      <Box sx={{ mt: 1 }}>
                        <Typography variant="body2">
                          <strong>Student Comments:</strong> {form.studentAcknowledgment.comments}
                        </Typography>
                      </Box>
                    )}
                  </Alert>
                  ...
                  <Alert severity="warning">
                    Student has not yet acknowledged this form
                  </Alert>
                )}
              </CardContent>
            </Card>
        ...
        <Grid item xs={12} md={4}>
          <Paper sx={{ p: 3 }}>
            ...
            <Alert severity="info" sx={{ mb: 2 }}>
              As a parent/guardian, you are acknowledging that you have been informed of this disciplinary action and understand the consequences outlined above.
            </Alert>
            ...
            <TextField
              ...
              variant="outlined"
              sx={{ mb: 2 }}
              placeholder="You may provide any comments, concerns, or requests for follow-up here..."
            />
            ...
            <Typography variant="caption" display="block" sx={{ mt: 1, textAlign: 'center' }}>
              This acknowledgment will be sent to the school administration and your child's teacher.
            </Typography>

            <Alert severity="warning" sx={{ mt: 2 }}>
              <Typography variant="body2">
                If you have concerns about this disciplinary action, please contact the school administration or request a meeting with the teacher.
              </Typography>
            </Alert>
          </Paper>
        </Grid>
      ...
      <Dialog open={confirmDialogOpen} onClose={() => setConfirmDialogOpen(false)}>
        <DialogTitle>Confirm Parent Acknowledgment</DialogTitle>
        <DialogContent>
          <Typography>
            Are you sure you want to submit your acknowledgment as the parent/guardian of {form.studentName}? This action cannot be undone.
          </Typography>
          <Box sx={{ mt: 2 }}>
            <Typography variant="subtitle2">Parent/Guardian Name:</Typography>
