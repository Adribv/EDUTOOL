...
const TeacherRemarksView = () => {
  ...
  // Mock children data - in real app, get from parent API
  const children = [
    { id: 'child1', name: 'John Doe', class: '7', section: 'A' },
    { id: 'child2', name: 'Jane Doe', class: '5', section: 'B' }
  ];
  ...
  const fetchForms = async () => {
    ...
    try {
      ...
      const response = await teacherRemarksAPI.getParentForms(selectedChild, params);
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      {!selectedChild && (
        <Box sx={{ textAlign: 'center', py: 4 }}>
          <Typography variant="h6" color="textSecondary">
            Please select a child to view teacher remarks.
          </Typography>
        </Box>
      )}
